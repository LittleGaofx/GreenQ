...
function showGameScoreLayer() {
    ...
    var score_text='得分&nbsp;&nbsp;';
    score_text+=deviation_time<13000?_gameScore:"<span style='color:red;'>"+_gameScore+"</span>";
    document.getElementById('GameScoreLayer-score').innerHTML = score_text;
    var bast = cookie('bast-score');
    if(deviation_time<13000){
...
function shareText(score) {
    ...
    if (deviation_time > 93000) {
        return '倒计时多了' + ((deviation_time / 1000) - 20).toFixed(2) + "s";
    }
    SubmitResults();
    if (score <= 49) return '此模式下不记录成绩';
    if (score <= 99) return '此模式下不记录成绩';
    if (score <= 149) return '此模式下不记录成绩';
    if (score <= 199) return '此模式下不记录成绩';
    return '此模式下不记录成绩';
}
