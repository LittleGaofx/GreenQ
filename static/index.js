...
function showGameScoreLayer() {
    ...
    score_text += "<span style='color:red;'>" + (__Time - _gameTimeNum).toString() + "</span>" + ' 秒<br>得分为 ';
    score_text += "<span style='color:red;'>" + _gameScore + "</span>";
    score_text += '<br>平均每秒点击 ';
    ...
    document.getElementById('GameScoreLayer-bast').innerHTML = '最佳得分 ' + "<span style='color:red;'>" + bast + "</span>";
...
function shareText(score) {
    ...
    if (deviation_time > __Time * 1000 + 3000) {
        return '实际时间比设置时间多了' + ((deviation_time / 1000) - __Time).toFixed(2) + "秒，本次成绩作废";
    }
    if (score <= 2.5 * __Time) return '太逊了';
    if (score <= 5 * __Time) return '有点东西嘛';
    if (score <= 7.5 * __Time) return '牛逼!';
    if (score <= 10 * __Time) return 'woc 真是太牛逼了!';
    return '诶诶诶你别激动嗷 冷静冷静';
}
